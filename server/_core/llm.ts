...
/** Opens an OpenAI-compatible server-sent event stream. The caller owns the body reader. */
export async function invokeLLMStream(params: Pick<InvokeParams, "messages" | "model" | "maxTokens" | "max_tokens">): Promise<Response> {
  assertApiKey();
  const payload: Record<string, unknown> = {
