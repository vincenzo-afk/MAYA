...
import { extractStreamMemories, inferStreamEmotion, streamMayaReply } from "./mayaBrain";

describe("Maya's emotional response helpers", () => {
  it("responds with care to language that needs gentleness", () => {
    expect(inferStreamEmotion("I feel lonely and overwhelmed today")).toMatchObject({
      label: "caring",
      intensity: 4,
      userMood: "needs gentleness",
    });
  });

  it("recognizes playful invitations", () => {
    expect(inferStreamEmotion("Let's play a fun game, Maya")).toMatchObject({
      label: "playful",
      userMood: "playful",
    });
  });
